...
function updateProfile(key,updatefield){
	...
	var jsonObject = JSON.stringify(
		  {"id":"C101","firstName":"John","lastName":"k","email":"vindy@gmail.com","mobile":"121-232-3435","dateOfBirth":1484116667527,"address":updatefield}
...
bot.dialog('/profile', [
	...
	function (session, results) {
		...
		session.send("Thank you for providing your details."); 
			session.endDialog();
    }
]);
...
intents.matches(/^yes?/i, [  
    function(session) {  
	 ...
	 var cust = arr[0];  
	 //var custJson = {"id":cust.id,"firstName":cust.firstName,"lastName":cust.lastName,"email":cust.email,"mobile":cust.mobile,"dateOfBirth":cust.dateOfBirth,"address": "?" };
	 //session.userData.custData=custJson;
...
intents.matches(/^please update/i, [  
    function(session) {  
    ...
    function(session, results) {  
		session.userData.sureaddress = results.response.entity;
		//console.log("my option ->"+session.userData.sureaddress);
		getCustomerDetails(session.userData.name);
...
intents.matches(/^ok/i, [  
    ...
    function(session, results) {  
		  session.send(" Thank you",results.response);
